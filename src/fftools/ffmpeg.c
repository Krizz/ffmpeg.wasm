...
/* ffmpeg.wasm: report transcode progress (0..1) and the current timestamp
 * (in AV_TIME_BASE units) back to JavaScript. Wired to Module.receiveProgress
 * in src/bind/ffmpeg/bind.js. */
EM_JS(void, send_progress, (double progress, double time), {
    Module.receiveProgress(progress, time);
});

/* ffmpeg.wasm: is_timeout returns 1 when the current run has been executing
 * for longer than the user-configured Module.timeout (milliseconds); -1
...
/* ffmpeg.wasm: compute fractional progress from the current pts and the
 * longest input duration, and forward it to JS via send_progress(). When
 * force_end is set the fraction is clamped to 1 (used for the final report). */
static void ffmpeg_send_progress(int64_t pts, int force_end)
{
    int64_t pts_abs = FFABS(pts);
    int64_t duration = -1;

    for (int i = 0; i < nb_input_files; i++) {
        int64_t d = input_files[i]->ctx->duration;
        if (d > duration)
            duration = d;
    }
    ...
    if (force_end)
        ...
        send_progress(0, (double)pts_abs);
}
...
static int transcode(Scheduler *sch)
{
    ...
    while (1) {
        ...
        /* ffmpeg.wasm: report progress to JS. transcode_ts is the current
         * output pts in AV_TIME_BASE units; we estimate completion against
         * the longest input duration. This is accurate for straight
         * transcodes; cases like -ss/-t trimming are still approximate. */
        ffmpeg_send_progress(transcode_ts, 0);
    }
...
/* ffmpeg.wasm: init_globals resets the file-scope state that ffmpeg_cleanup()
 * does not, so that ffmpeg() can be invoked repeatedly within the same wasm
 * instance. In the original CLI this is unnecessary because every run is a
 * fresh process; here the module is long-lived. ffmpeg_cleanup() already frees
 * and NULLs input_files/output_files/filtergraphs/decoders, but it leaves the
 * associated counters and the signal/exit flags set, which we reset here. */
...
finish:
    ...
    /* ffmpeg.wasm: surface the exit code to JS (read as Module.ret in
     * src/bind/ffmpeg/bind.js). Unlike the original CLI we return cleanly
     * instead of exiting the process, so the wasm runtime stays alive for the
     * next call. */
    EM_ASM({
        Module.ret = $0;
    }, ret);
